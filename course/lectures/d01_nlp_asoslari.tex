...
\begin{frame}{Til Qanday Tashkil Etilgan? Linguistik Darajalar Stek}
  ...
  \begin{tikzpicture}[
    ...
    \node[top] (p)  {5.~Pragmatika / Diskurs \;---\; kontekstdagi ma'no; gaplararo aloqa};
    \node[lvl, below=2mm of p]  (s)  {4.~Semantika \;---\; so'z ma'nosi va ko'p ma'nolilik};
    \node[lvl, below=2mm of s]  (sy) {3.~Sintaksis \;---\; gap tuzilishi (SOV / SVO)};
    \node[lvl, below=2mm of sy] (m)  {2.~Morfologiya \;---\; so'z tuzilishi
          (\textit{kitob+lar+imiz+dan})};
    \node[lvl, below=2mm of m]  (ph)
          {\textcolor{halfgray}{1.~Fonologiya \;---\; tovush tizimi
          (matnli NLP uchun ko'rinmas)}};
  \end{tikzpicture}
  \end{center}
  \vspace{3pt}
  \footnotesize
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      \begin{tcolorbox}[warnbox,title=BoW va TF-IDF muammosi,top=2pt,bottom=2pt]
        \footnotesize
        BoW har shakl = \textbf{alohida token}:\\[3pt]
        \texttt{"kitob"} $\ne$ \texttt{"kitoblar"}\\
...
\begin{frame}{Sintaksis va Semantika: Tartib Muhim, Ma'no Murakkab}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{tcolorbox}[defbox,title=SOV vs SVO,top=2pt,bottom=2pt]
        \footnotesize
        O'zbek: \textbf{S--O--V}\\
...
\begin{frame}{Pragmatika, Diskurs va NLP Qamrovi}
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      ...
      TF-IDF ``zo'r'' da \textbf{musbat} ko'radi ---
      kontekstni bilmaydi.\\
      BERT va LLM kontekstni hisobga oladi (L13--L16).\\[5pt]
      \textbf{Diskurs} (gaplararo aloqa):\\[3pt]
      \small
    ...
    \begin{column}{0.46\textwidth}
      ...
      \begin{tabular}{p{1.8cm}p{3.0cm}}
        \toprule
        Daraja & Bu kursdagi qurol \\
        \midrule
        Fonologiya  & \textcolor{halfgray}{Speech --- tashqarida} \\
...
\begin{frame}{Vazifalar Taksonomiyasi: To'rt Asosiy Tur va Kurs Yo'l Xaritasi}
  \small
  \begin{tabular}{p{3.0cm}p{2.7cm}p{5.3cm}}
    \toprule
    \textbf{Tur} & \textbf{Chiqarma} & \textbf{Misollar va ma'ruza} \\
    \midrule
    ...
    \textbf{Strukturaviy bashorat} &
      ...
      chiqarma tuzilishiga ega (skalyar emas) \\
    \bottomrule
  \end{tabular}
  \vspace{5pt}
  \begin{tcolorbox}[okbox,title=Muhim nuqta,top=2pt,bottom=2pt]
    \footnotesize
    Klassifikatsiya: \texttt{sigmoid}/\texttt{softmax} bitta chiqarma.
...
\begin{frame}{Klassik NLP va End-to-End Neyron NLP: Kurs Yo'li}
  \begin{columns}[T]
    \begin{column}{0.46\textwidth}
    ...
    \begin{column}{0.50\textwidth}
      ...
      \begin{tikzpicture}[
        ...
        \node[fin, below=2mm of enc] (out) {Chiqarma (label / matn)};
        \draw[arr] (inp)--(emb);
        \draw[arr] (emb)--(enc);
        \draw[arr] (enc)--(out);
      \end{tikzpicture}
